...
public class AuthServiceSWTests extends AppCompatActivity {

    private static final String FILENAME_OUTPUT = "/outputs/authentication-sw-%s-output-%s.json";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_test_authentication_secure_world);

        /* initializing unlockButton */
        startTestButton = findViewById(R.id.startTest);

        /* initializing testAttestation */
        testAuthentication = new CheckBox[NUM_OF_TESTS];
        for (int i = 0; i < NUM_OF_TESTS; i++) {
            testAuthentication[i] = findViewById(R.id.checkDescription + (i + 1));
        }
        resetTests();
        ...
        startTestButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                new Thread(() -> {
                    ...
                    if (testAuthentication[0].isChecked()) {
                        output[0] = tWalletUtils.storeCredentialsBenchmark(iterations);

                        runOnUiThread(() -> progressBar.setProgress(iterations));
                        testAuthentication[0].setText(testAuthentication[0].getText() + "\t \u2713");
                        writeIntoFile("store", output[0], iterations);
                    } else progressBar.setProgress(progressBar.getProgress() + iterations);


                    /* Test Credential Retrieval*/
                    if (testAuthentication[1].isChecked()) {
                        output[0] = tWalletUtils.loadCredentialsBenchmark(iterations);

                        runOnUiThread(() -> progressBar.setProgress(iterations * 2));
                        testAuthentication[1].setText(testAuthentication[1].getText() + "\t \u2713");
                        writeIntoFile("load", output[0], iterations);
                    } else progressBar.setProgress(progressBar.getProgress() + iterations);


                    /* Test Credential Deletion*/
                    if (testAuthentication[2].isChecked()) {
                        output[0] = tWalletUtils.deleteCredentialsBenchmark(iterations);

                        runOnUiThread(() -> progressBar.setProgress(iterations * 3));
                        testAuthentication[2].setText(testAuthentication[2].getText() + "\t \u2713");
    ...
    private void resetTests() {
        testAuthentication[0].setText("Store Credentials");
        testAuthentication[1].setText("Load Credentials");
        testAuthentication[2].setText("Delete Credentials");
    }
