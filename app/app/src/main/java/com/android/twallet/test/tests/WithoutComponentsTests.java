...
import com.android.twallet.test.web3J.Web3jHandler;
...
public class WithoutComponentsTests extends AppCompatActivity {

    private static final String NEW_WALLET_DIRECTORY = "/wallets";
    private static final String FILENAME_OUTPUT = "/outputs/operations-%s-normal-output-%s.json";
    private static final String TEST_WALLET = "/UTC--2021-07-27T13-29-03.408186000Z--a37c7fa57fce2160cc109d43968c27a57e4f1b23.json";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_test_without_components);
                ...
                new Thread(() -> {
                    ...
                    if (testOperations[0].isChecked() || testOperations[1].isChecked() || testOperations[2].isChecked()) {
                        ...
                        try {
                            ...
                            for (int i = 0; i < iterations; i++) {
                                ...
                                Web3jHandler.createWallet("pass", newWalletPath);
                        ...
                        try {
                            for (int i = 0; i < iterations; i++) {
                                long startTime = System.currentTimeMillis();
                                Web3jHandler.loadCredentials("pass", Objects.requireNonNull(directory.listFiles())[0].getPath());
                                values[i] = System.currentTimeMillis() - startTime;
                        ...
                        try {
                            for (int i = 0; i < iterations; i++) {
                                long startTime = System.currentTimeMillis();
                                Web3jHandler.deleteWallet("pass", Objects.requireNonNull(directory.listFiles())[0].getPath());
                                values[i] = System.currentTimeMillis() - startTime;
                    ...
                    if (testOperations[4].isChecked()) {
                        try {
                            for (int i = 0; i < iterations; i++) {
                                long startTime = System.currentTimeMillis();
                                Web3jHandler.transaction(address, 0.01);
                                values[i] = System.currentTimeMillis() - startTime;
                    ...
                    if (testOperations[5].isChecked()) {
                        try {
                            for (int i = 0; i < iterations; i++) {
                                long startTime = System.currentTimeMillis();
                                Web3jHandler.getPastTransactions();
                                values[i] = System.currentTimeMillis() - startTime;
    ...
    private String setupWallet() {
        String address = "";
        try {
            address = Web3jHandler.createWallet("pass", getFilesDir().getPath());
            Web3jHandler.loadCredentials("pass123", getFilesDir().getPath() + TEST_WALLET);
            Web3jHandler.setTxHistory();
    ...
    private void destroyWallet(String address) {
        try {
            ...
            File wallet = Arrays.stream(Objects
                    .requireNonNull(getFilesDir().listFiles()))
                    .findFirst()
                    .filter(file -> file.getName().contains(address.substring(2))).get();
            Web3jHandler.deleteWallet("pass", wallet.getPath());
